...
def run_training_pipeline(model_type: list = ["logistic_regression"], params: dict = {}):
    ...
    return {"best_models": best_models, "champion_model": champion_model, "test_metrics": test_metrics}
...
if __name__ == "__main__":
    ...
    champion_model = run_training_pipeline(
        model_type=list_of_models, params={"save_output": False, "plot_metrics": True, "save_split_data": True}
    )
    logger.info(
        f"Champion   model based on F1 score: {champion_model['champion_model'][0]} with F1 score = {champion_model['champion_model'][1]['best_score']:.4f}"
    )
